...
import type {
  AvDatePickerAnimationState,
  AvDatePickerCloseReason,
  AvDatePickerOpenOrigin,
  AvDatePickerPlacementAxis,
} from './date-picker.utils';
import { AV_DATE_PICKER_ENTER_MS, AV_DATE_PICKER_EXIT_MS } from './date-picker.utils';

/** Fallback when exit CSS animation does not fire (e.g. reduced motion). */
const EXIT_ANIMATION_FALLBACK_MS = AV_DATE_PICKER_EXIT_MS + 50;

export interface AvDatePickerPanelConfig {
  templateRef: TemplateRef<unknown>;
  viewContainerRef: ViewContainerRef;
  getPositions: () => ConnectedPosition[];
  shouldFlip: () => boolean;
  setPlacementAxis: (axis: AvDatePickerPlacementAxis) => void;
}

@Injectable()
export class AvDatePickerContext {
  readonly isOpen = signal(false);
  readonly isVisible = signal(false);
  readonly animationState = signal<AvDatePickerAnimationState>('idle');
  ...
  readonly value = signal<AvDatePickerValue>(null);
  ...
  private valueChangeHandler: ((value: AvDatePickerValue) => void) | null = null;
  private touchedHandler: (() => void) | null = null;
  private triggerElement: HTMLElement | null = null;
  private panelConfig: AvDatePickerPanelConfig | null = null;
  private openedBy: AvDatePickerOpenOrigin = 'program';
  private closeReason: AvDatePickerCloseReason;
  private detachOverlayHandler: (() => void) | null = null;
  private restoreFocusHandler: ((origin: AvDatePickerOpenOrigin) => void) | null = null;

  registerOpenChange(handler: (open: boolean) => void): void {
    this.openChangeHandler = handler;
  }

  registerValueChange(handler: (value: AvDatePickerValue) => void): void {
  ...
  registerPanel(config: AvDatePickerPanelConfig): void {
  ...
  registerOverlayHandlers(handlers: {
    detach: () => void;
    restoreFocus: (origin: AvDatePickerOpenOrigin) => void;
  }): void {
    this.detachOverlayHandler = handlers.detach;
    this.restoreFocusHandler = handlers.restoreFocus;
  }

  getPanelConfig(): AvDatePickerPanelConfig | null {
  ...
  getOpenedBy(): AvDatePickerOpenOrigin {
    return this.openedBy;
  }

  getCloseReason(): AvDatePickerCloseReason {
    return this.closeReason;
  }

  setOpenedBy(origin: AvDatePickerOpenOrigin): void {
  ...
  open(origin: AvDatePickerOpenOrigin = 'program'): void {
  ...
  close(reason?: AvDatePickerCloseReason): void {
  ...
  /** Apply a calendar day selection to the picker value and optionally close. */
  notifyCalendarSelection(date: CalendarDate): void {
    const next = mergeCalendarDateIntoValue(this.value(), date);
    this.value.set(next);
    this.valueChangeHandler?.(next);
...
/** Preserve time fields when replacing the calendar day. */
export function mergeCalendarDateIntoValue(
  current: AvDatePickerValue,
  date: CalendarDate,
): AvDatePickerValue {
