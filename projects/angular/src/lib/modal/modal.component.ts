...
export class AvModalComponent {
  ...
  /** Controls whether the modal is open. Supports two-way binding with `[(open)]`. */
  ...
  readonly placement = input<AvModalPlacement>('auto');

  /** Scroll behavior for long content. */
  readonly scroll = input<AvModalScroll>('inside');

  /** Maximum width / layout preset. */
  readonly size = input<AvModalSize>('md');

  /** Backdrop visual variant. */
  readonly backdrop = input<AvModalBackdropVariant>('opaque');
  ...
  protected readonly backdropClasses = computed(() =>
    avModalResolveBackdropClasses(this.backdrop(), this.backdropClass()).join(' '),
  );
