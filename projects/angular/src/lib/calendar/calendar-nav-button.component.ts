...
@Component({
  ...
  selector: 'button[av-calendar-nav-button]',
  template: `
    <ng-content>
      @if (slot() === 'previous') {
        <svg
...
export class AvCalendarNavButtonComponent {
  private readonly context = inject(AvCalendarContext);
  private readonly intl = inject(AvCalendarIntl);

  /** Navigation direction slot. */
  readonly slot = input.required<'previous' | 'next'>();

  protected readonly classes = computed(() => avCalendarNavButtonClasses());
  protected readonly iconClasses = computed(() => avCalendarNavButtonIconClasses());
